...
class ManifestDataset(Dataset):
    ...
    def _prepare(self, img: Image.Image) -> Image.Image:
        """Equalise resampling artifacts then crop — never stretch.

        Strategy:
          1. Randomly rescale to a random intermediate size (augment) or fixed
             intermediate size (val). This ensures both AI and real images go
             through the same resampling operation, eliminating the size-bias.
          2. Pad with reflection if the image is smaller than image_size.
          3. Random crop (train) or centre crop (val) to image_size.
        """
        s = self.image_size

        # Step 1: resampling augmentation — rescale to a random intermediate size.
        # Both small AI images and large real images pass through this, so the
        # resampling artifact distribution is equalised across classes.
        if self.augment:
            # Scale so the shorter side lands between 1.0x and 1.5x image_size.
            scale = random.uniform(1.0, 1.5)
        else:
            # Val: always scale shorter side to exactly 1.15x for a clean centre crop.
            scale = 1.15

        target_short = int(s * scale)
        w, h = img.size
        ...
        # Step 2: pad with reflection if still smaller than crop size.
        w, h = img.size
        if w < s or h < s:
            pad_w = max(0, s - w)
            pad_h = max(0, s - h)
            # Use ImageOps.expand with reflect padding via numpy
            import numpy as np
            arr = np.array(img)
            arr = np.pad(arr,
                         ((0, pad_h), (0, pad_w), (0, 0)),
    ...
    def __getitem__(self, idx: int):
        ...
        # Mild spatial augmentations before cropping.
        if self.augment:
...
def main() -> int:
    ...
    parser.add_argument("--manifest-root", type=str, default="",
                        help="If set, rebuilds manifest from this dataset root before each epoch.")
    ...
    parser.add_argument("--finetune-all", action="store_true",
                        help="Unfreeze entire RGB backbone for full fine-tuning.")
    ...
    if out_path.exists():
        try:
            ...
            print(f"Resumed from checkpoint: {out_path} (epoch {start_epoch})")
    ...
    try:
        while True:
            ...
            # Rebuild manifest from disk before each epoch so newly scraped
            # images are picked up automatically.
            if args.manifest_root:
                import subprocess
                subprocess.run([
                    sys.executable,
                    str(Path(__file__).parent / "build_manifest.py"),
            ...
            val_loader   = DataLoader(val_ds,   batch_size=args.batch_size, shuffle=False,
                                      num_workers=args.num_workers, persistent_workers=args.num_workers > 0, pin_memory=True)
            print(f"Epoch {epoch} | train={len(train_items)} val={len(val_items)}")
